...
* `GET /api/clients/autocomplete?query={query}` получить список автозаполнения клиентов по запросу. Параметры, передаваемые в URL:
    * `query={query}` строка для поиска клиентов по имени, фамилии или отчеству.
* `GET /#id клиента` получить клиента по его ID из hash-части URL. Пример:
    ```
    GET /#12345
    ```
    Этот метод возвращает HTML файл с открытым модальным окном редактирования клиента.
